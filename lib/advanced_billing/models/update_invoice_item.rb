...
module AdvancedBilling
  # A line item change for a draft ad hoc invoice. Supports the same attributes
  # as line items on invoice creation, plus `uid` and `_destroy` for updating or
  ...
  class UpdateInvoiceItem < BaseModel
    ...
    # Unique identifier of an existing line item on the invoice. When provided,
    # the matching line item is updated with the submitted attributes. When
    # omitted, a new line item is added to the invoice.
    # @return [String]
    attr_accessor :uid

    # Set to `true` together with `uid` to remove the matching line item from
    # the invoice. Line items not referenced in the request remain unchanged.
    # @return [TrueClass | FalseClass]
    ...
    # @param [UpdateInvoiceItem | Hash] The value against the validation is performed.
