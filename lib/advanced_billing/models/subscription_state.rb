...
module AdvancedBilling
  # The state of a subscription. * **Live States**     * `active` - A normal,
  # active subscription. It is not in a trial and is paid and up to date.     *
  # `assessing` - An internal (transient) state that indicates a subscription is
  # in the middle of periodic assessment. Do not base any access decisions in
  # your app on this state, as it may not always be exposed.     * `pending` -
  # An internal (transient) state that indicates a subscription is in the
  # creation process. Do not base any access decisions in your app on this
  # state, as it may not always be exposed.     * `trialing` - A subscription in
  # trialing state has a valid trial subscription. This type of subscription may
  # transition to active once payment is received when the trial has ended.
  # Otherwise, it may go to a Problem or End of Life state.     * `paused` - An
  # internal state that indicates that your account with Advanced Billing is in
  # arrears. * **Problem States**     * `past_due` - Indicates that the most
  # recent payment has failed, and payment is past due for this subscription. If
  # you have enabled our automated dunning, this subscription will be in the
  ...
  # cannot be fixed by the Customer. For example, a Soft Fail may result from a
  # timeout at the gateway or incorrect credentials on your part. The
  # subscriptions should be retried automatically. An interface is being built
  # for you to review problems resulting from these events to take manual action
  ...
  class SubscriptionState
    SUBSCRIPTION_STATE = [
      ...
      # TODO: Write general description for FAILED_TO_CREATE
      FAILED_TO_CREATE = 'failed_to_create'.freeze,

      # TODO: Write general description for TRIALING
      TRIALING = 'trialing'.freeze,

      # TODO: Write general description for ASSESSING
      ASSESSING = 'assessing'.freeze,

      # TODO: Write general description for ACTIVE
      ACTIVE = 'active'.freeze,

      # TODO: Write general description for SOFT_FAILURE
      SOFT_FAILURE = 'soft_failure'.freeze,
