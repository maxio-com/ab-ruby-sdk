...
| `site_id` | `Integer` | Optional | ID of the site to which the debit note belongs. |
| `customer_id` | `Integer` | Optional | ID of the customer to which the debit note belongs. |
| `subscription_id` | `Integer` | Optional | ID of the subscription that generated the debit note. |
| `number` | `Integer` | Optional | A unique, identifier that appears on the debit note and in places it is referenced. |
| `sequence_number` | `Integer` | Optional | A monotonically increasing number assigned to debit notes as they are created. |
