...
RSpec.describe 'Awaiting sign up subscription' do
  ...
  it 'creates subscription with coupon and metered component' do
    ...
    expect(subscription.current_period_ends_at.to_date).to be_between(Date.today, Date.today + 1)
    expect(subscription.current_period_started_at.to_date).to be_between(Date.today - 1, Date.today)
